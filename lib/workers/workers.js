    ...
    workers.alertUsertoStatusChange = (newCheckData => {
        ...
        helpers.sendTwilioSms(newCheckData.userPhone, msg, (resMessage, statusCode) => {
            if (statusCode === 200 || statusCode === 201) {
                ...
                console.log("Error: could not send sms to user who had a state change in their check.", resMessage);
            }
        })
    })
};
            ...
            logs.forEach(logName => {
                ...
                _logs.compress(logId, newFileId, (err) => {
                    if (!err){
                        ...
                        console.log(`Error compressing one of the logFiles: ${err}`);
                    }
                })
            })
        } else {    
            console.log("Error: could not find andy logs to rotate")
        }
    })
};
